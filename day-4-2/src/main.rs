...
// section 1 is overlapping with section 2 if x >= a && x <= b || y <= b && y >= a
fn is_contained(a: &str, b: &str) -> bool {
    let (a_lower, a_upper) = split_section(a);
    let (b_lower, b_upper) = split_section(b);
    ((a_lower >= b_lower && a_lower <= b_upper) || (a_upper <= b_upper && a_upper >= b_lower)) || ((b_lower >= a_lower && b_lower <= a_upper) || (b_upper <= a_upper && b_upper >= a_lower))
}
